...
<body>
              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header">
                    <h4><?php echo $_GET['name']; ?> Students</h4>
                    <a href="add-classroom-student.php?id=<?php echo $_GET['id']; ?>&name=<?php echo $_GET['name']; ?>" class="btn btn-primary pull-right">Add Students</a>
                  </div>
                  <div class="card-body">
  ...
  <script type="module">
    ...
    const studentclassroomsQuery = query(studentclassroomsCol, where('classroomID', '==', '<?php echo $_GET['id']; ?>'));
    ...
    const studentclassroomDocs = studentclassroomSnapshot.forEach((scsnap) => {    
      const userDocs = userSnapshot.forEach((ssnap) => {
        if (scsnap.data().studentID == ssnap.id) {
          var tableRow = '<tr>';
          tableRow += '<td>' + index + '</td>';
          tableRow += '<td>' + ssnap.data().displayName + '</td>';
          tableRow += '<td>' + ssnap.data().email + '</td>';
          tableRow += '<td>' + ssnap.data().phoneNumber + '</td>';
          tableRow += (typeof(ssnap.data().profilePicture) != "undefined") ? '<td><img alt="image" src="' + ssnap.data().profilePicture + '" width="75"></td>' : '<td></td>';
          // tableRow += '<td><a href="#" class="btn btn-primary">Detail</a></td>';
          tableRow += '</tr>';

          var student_table = document.getElementById('student_table');
          student_table.innerHTML += tableRow;
          index++;
        }
      });  
    });
