...
<head>
  ...
  <title>OASIS- Online Access School Information System</title>
  <!-- General CSS Files -->
  ...
  <link rel='shortcut icon' type='image/x-icon' href='assets/img/oasis_logo.png' />
</head>
...
<body>
        ...
        <ul class="navbar-nav navbar-right">
          ...
          <li class="dropdown"><a href="#" data-toggle="dropdown"
            ...
            <div class="dropdown-menu dropdown-menu-right pullDown">
              ...
              <a href="#" onclick="logout()" class="dropdown-item has-icon text-danger"> <i class="fas fa-sign-out-alt"></i>
                Logout
              </a>
            </div>
          </li>
      ...
      <div class="main-sidebar sidebar-style-2">
        <aside id="sidebar-wrapper">
          <div class="sidebar-brand">
            <a href="index.html"> <img alt="image" src="assets/img/oasis_logo.png" class="header-logo" /> <span
                class="logo-name">OASIS</span>
            </a>
          </div>
          <ul class="sidebar-menu">
            <li class="dropdown">
              <a href="#" class="menu-toggle nav-link has-dropdown"><i data-feather="users"></i><span>Profile</span></a>
              <ul class="dropdown-menu">
                <li><a class="nav-link" href="index.html">Teacher</a></li>
                <li><a class="nav-link" href="student.html">Student</a></li>
              </ul>
            </li>
            <li class="dropdown active">
              <a href="#" class="menu-toggle nav-link has-dropdown"><i data-feather="monitor"></i><span>Classroom</span></a>
              <ul class="dropdown-menu">
                <li><a class="nav-link" href="classroom.html">View Classrooms</a></li>
                <li><a class="nav-link" href="add-classroom.html">Add Classroom</a></li>
              </ul>
            </li>
            <li class="dropdown">
              <a href="#" class="menu-toggle nav-link has-dropdown"><i data-feather="book-open"></i><span>Subject</span></a>
              <ul class="dropdown-menu">
                <li><a class="nav-link" href="subject.html">View Subjects</a></li>
                <li><a class="nav-link" href="add-subject.html">Add Subject</a></li>
              </ul>
            </li>
            <li class="dropdown">
              <a href="#" class="menu-toggle nav-link has-dropdown"><i data-feather="monitor"></i><span>Classes Info</span></a>
              <ul class="dropdown-menu">
                <li><a class="nav-link" href="teachersubjectclassroom.html">View Classes Info</a></li>
                <li><a class="nav-link" href="add-teachersubjectclassroom.html">Add Classes Info</a></li>
              </ul>
            </li>
            <!-- <li class="dropdown"> -->
              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header">
                    <h4>Add Students to <?php echo $_GET['name']; ?></h4>
                  </div>
                  <div class="card-body">
                  ...
                  <div class="card-footer text-right">
                    <button class="btn btn-primary mr-1" onclick="addStudents()">Submit</button>
                  </div>
  ...
  <script type="module">
    ...
    import { getFirestore, collection, getDocs, getDoc, query, where, orderBy, addDoc } from "https://www.gstatic.com/firebasejs/9.0.0/firebase-firestore.js";
    ...
    const usersQuery = query(usersCol, where('type', '==', 'Student'));
    const studentclassroomsCol = collection(db, 'studentclassrooms');
    const studentclassroomsQuery = query(studentclassroomsCol);

    onAuthStateChanged(auth, user => {
        if (user != null) {
          
        } else {
          window.location.href = "/adminpanel/auth-login.html";
        }
    });

    const userSnapshot = await getDocs(usersQuery);
    const studentclassroomSnapshot = await getDocs(studentclassroomsQuery);

    userSnapshot.forEach((ssnap) => {
      var studentInClass = false;  

      studentclassroomSnapshot.forEach((scsnap) => {
        if(ssnap.id == scsnap.data().studentID) {
          studentInClass = true;
        }
      });

      if (studentInClass == false) {
        var checkboxElem = '<div class="form-check"><input class="form-check-input student-checkbox" type="checkbox" id="student' + ssnap.id + '" value="' + ssnap.id + '">';
        checkboxElem += '<label class="form-check-label" for="student' + ssnap.id + '">' + ssnap.data().displayName + '</label></div>';

        var student_list = document.getElementById('student_list');
        student_list.innerHTML += checkboxElem;
      }   
    });

    window.addStudents = async function addStudents() {
      var checkedStudent = [];
      var inputElements = document.getElementsByClassName('student-checkbox');
      
      for(var i=0; inputElements[i]; ++i){
        if(inputElements[i].checked){
          checkedStudent.push(inputElements[i].value);
        }
      }

      for(var i=0; checkedStudent[i]; ++i){
        const newDoc = await addDoc(studentclassroomsCol, {
          classroomID: "<?php echo $_GET['id']; ?>",
          studentID: checkedStudent[i],
        });
      }

      alert('Students added!');
      window.location.href = "/adminpanel/classroom-students.php?id=<?php echo $_GET['id']; ?>&name=<?php echo $_GET['name']; ?>";
    }
